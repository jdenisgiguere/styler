                ...
                items:
                    [{
                        ...
                        listeners: {
                            "click": function(btn, pressed) {
                                var feature = this.feature;
                                var bufferSizeCombo = btn.findParentByType("panel").findByType("combo")[0];
                                var bufferSize = bufferSizeCombo.getValue();
                                var bufferFeature = feature.clone();
                                var wkt = new OpenLayers.Format.WKT();
                                var json = new OpenLayers.Format.JSON();
                                var bJson, bWkt, bGeometry;
                                if (pressed) {
                                    OpenLayers.Request.POST({
                                        url: GEOR.config.PATHNAME + "/ws/buffer/" + bufferSize,
                                        data: wkt.extractGeometry(feature.geometry),
                                        success: function(response) {
                                            bJson = json.read(response.responseText);
                                            bWkt = bJson['geometry'];
                                            bGeometry = wkt.read(bWkt).geometry;
                                            bufferFeature.geometry = bGeometry;
                    ...
                    },
                    {
                        ...
                        store: {
                            ...
                            data: [[10, '10 m'], [100, '100 m'], [1000, '1 km'],[10000, '10 km']]
                        },
